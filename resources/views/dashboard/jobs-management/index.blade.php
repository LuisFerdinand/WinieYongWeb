          ...
          <div class="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
            <div class="flex flex-col items-center justify-between px-4 pt-4 pb-2 space-y-3 lg:flex-row lg:space-y-0 lg:space-x-4">
                <div class="w-full lg:w-1/2">
                    <form action="{{ route('jobs-management.index') }}" method="GET">
                        <div class="flex items-center w-full mx-auto mb-0 sm:max-w-screen-sm space-x-2 overflow-hidden">
                            ...
                            <div class="flex-shrink-0">
                                <a href="{{ route('jobs-management.index') }}" class="bg-gray-300 hover:bg-gray-500 text-black px-4 py-2 rounded-lg">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
              ...
              <div class="flex items-stretch justify-between flex-shrink-0 w-full space-y-0 lg:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                <a href="{{ route('jobs-management.create') }}">
                ...
                <div class="flex items-center space-x-3 md:w-auto">
                      ...
                      <div id="dropdownSort1" class="z-50 hidden w-40 divide-y divide-gray-100 rounded-lg bg-white shadow dark:bg-gray-700" data-popper-placement="bottom">
                        <ul class="p-2 text-left font-medium text-gray-500 dark:text-gray-400 text-xs" aria-labelledby="sortDropdownButton">
                            ...
                            <li class="border-t border-gray-200 my-1"></li>
                            <li>
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'positions_asc']) }}" 
                                   class="group inline-flex w-full items-center rounded-md px-3 py-2 text-xs {{ request('sort') === 'positions_asc' ? 'bg-gray-100 text-gray-900' : 'text-gray-500' }} hover:bg-gray-100 hover:text-gray-900">
                                    Total Positions ↑ (Least)
                                    @if(request('sort') === 'positions_asc')
                                        <svg class="ml-2 h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                                        </svg>
                                    @endif
                            ...
                            <li>
                                <a href="{{ request()->fullUrlWithQuery(['sort' => 'positions_desc']) }}" 
                                   class="group inline-flex w-full items-center rounded-md px-3 py-2 text-xs {{ request('sort') === 'positions_desc' ? 'bg-gray-100 text-gray-900' : 'text-gray-500' }} hover:bg-gray-100 hover:text-gray-900">
                                    Total Positions ↓ (Most)
                                    @if(request('sort') === 'positions_desc')
                                        <svg class="ml-2 h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                                        </svg>
                                    @endif
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
            <thead class="bg-gray-800 text-white text-xs uppercase dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="pr-6 pl-2 py-3">Title</th>
                    <th scope="col" class="pr-6 pl-2 py-4">Description</th>
                    <th scope="col" class="pr-6 pl-2 py-4">Total Positions</th>
                    <th scope="col" class="pr-6 pl-2 py-4">Requirements</th>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-900 @if($loop->last) !border-0 @endif">
                    ...
                    <th class="flex items-center pr-6 pl-2 py-4 dark:text-white">
                        @if ($job->job_image)
                        <img class="w-14 h-14 object-contain rounded border-gray-500 border md:w-16 md:h-16" src="{{ asset('storage/'.$job->job_image) }}" alt="">
                        @elseif($job->job_image_url)
                        <img class="w-14 h-14 object-contain rounded border-gray-500 border md:w-16 md:h-16" src="{{ $job->job_image_url }}" alt="">
                        @else
                        ...
                        <div class="ps-3 w-44">
                            <div class="text-lg font-semibold leading-tight">{{ $job->job_title }}</div>
                            <hr class="border-gray-500 my-1">
                            <div class="font-semibold text-md text-gray-600">{{ $job->job_department }}</div>
                            <div class="font-medium text-md text-gray-500 italic">{{ $job->job_work_type }}</div>
                        </div>
                    </th>
                    ...
                    <td class="border-x px-6 py-4">
                        <div class="flex items-center justify-center gap-0.5 font-medium">
                            {{ $job->job_total_positions }}
                        </div>
                    </td>
                    <td class="border-x px-6 py-4">
                        <ul class="font-normal space-y-1 list-disc">
                            @foreach(array_filter(explode('.', $job->job_requirements)) as $requirement)
                    ...
                    <td class="border-x px-2 py-4 @if($loop->last) rounded-br-lg @endif">
                        <div class="flex items-center justify-center gap-2">
                            <a href=
                            "{{ route('jobs-management.edit', $job->job_slug) }}" class="text-yellow-500 font-bold"
                            > 
                            ...
                            <form action=
                            "{{ route('jobs-management.destroy', $job->job_slug) }}"
...
<script>
    ...
    //                 const editUrl = `/dashboard/services/machinery-jobs/jobs-management/${jobData.job_slug}/edit`;
    //                 const deleteUrl = `/dashboard/services/machinery-jobs/jobs-management/${jobData.job_slug}`;
    //                 let imageContent = "";
    //                 if (jobData.job_image) {
    //                     imageContent = `<img class="w-full dark:hidden rounded-md object-cover" src="{{ asset('storage/') }}${jobData.job_image}" alt="" />`;
    //                 } else if (jobData.job_image_url) {
    //                     imageContent = `<img class="w-full dark:hidden rounded-md object-cover" src="${jobData.job_image_url}" alt="" />`;
    //                 } else {
    ...
    //                                             <a href="#" class="text-5xl font-extrabold leading-tight text-gray-900 hover:underline dark:text-white justify-start ">${ jobData.job_title }</a>
    //                                         </div>
    //                                     </div>
