          ...
          <div class="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
            <div class="flex flex-col items-center justify-between px-4 pt-4 pb-2 space-y-3 lg:flex-row lg:space-y-0 lg:space-x-4">
                <div class="w-full lg:w-1/2">
                    <form action="{{ route('parts-management.index') }}" method="GET">
                        <div class="flex items-center w-full mx-auto mb-0 sm:max-w-screen-sm space-x-2 overflow-hidden">
                            ...
                            <div class="flex-shrink-0">
                                <a href="{{ route('parts-management.index') }}" class="bg-gray-300 hover:bg-gray-500 text-black px-4 py-2 rounded-lg">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
              ...
              <div class="flex items-stretch justify-between flex-shrink-0 w-full space-y-0 lg:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                <a href="{{ route('parts-management.create') }}">
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 ">
            <thead class="bg-gray-800 text-white text-xs uppercase dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="pr-6 pl-2 py-4">Contact</th>
                    <th scope="col" class="pr-6 pl-2 py-4">Location</th>
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 text-gray-900 @if($loop->last) !border-0 @endif">
                    ...
                    <th class="flex items-center pr-6 pl-2 py-4 dark:text-white">
                        @if ($part->part_image)
                        <img class="w-14 h-14 object-contain rounded border-gray-500 border md:w-16 md:h-16" src="{{ asset('storage/'.$part->part_image) }}" alt="">
                        @elseif($part->part_image_url)
                        <img class="w-14 h-14 object-contain rounded border-gray-500 border md:w-16 md:h-16" src="{{ $part->part_image_url }}" alt="">
                        @else
                        ...
                        <div class="ps-3 w-44">
                            <div class="text-lg font-semibold leading-tight">{{ $part->part_name }}</div>
                            <hr class="border-gray-500 my-1">
                            <div class="font-semibold text-md text-gray-600">{{ $part->part_category }}</div>
                            <div class="font-medium text-md text-gray-500 italic">{{ $part->part_work_type }}</div>
                        </div>
                    </th>
                    ...
                    <td class="border-x px-6 py-4">
                        <div class="flex items-center justify-center gap-0.5 font-medium">
                            Rp{{ number_format($part->part_price, 0, ',', '.') }}
                        </div>
                    </td>
                    ...
                    <td class="border-x px-2 py-4 @if($loop->last) rounded-br-lg @endif">
                        <div class="flex items-center justify-center gap-2">
                            <a href=
                            "{{ route('parts-management.edit', $part->part_slug) }}" class="text-yellow-500 font-bold"
                            > 
                            ...
                            <form action=
                            "{{ route('parts-management.destroy', $part->part_slug) }}"
...
<script>
    ...
    //                 const editUrl = `/dashboard/services/machinery-jobs/parts-management/${jobData.part_slug}/edit`;
    //                 const deleteUrl = `/dashboard/services/machinery-jobs/parts-management/${jobData.part_slug}`;
    //                 let imageContent = "";
    //                 if (jobData.part_image) {
    //                     imageContent = `<img class="w-full dark:hidden rounded-md object-cover" src="{{ asset('storage/') }}${jobData.part_image}" alt="" />`;
    //                 } else if (jobData.part_image_url) {
    //                     imageContent = `<img class="w-full dark:hidden rounded-md object-cover" src="${jobData.part_image_url}" alt="" />`;
    //                 } else {
    ...
    //                                             <a href="#" class="text-5xl font-extrabold leading-tight text-gray-900 hover:underline dark:text-white justify-start ">${ jobData.part_name }</a>
    //                                         </div>
    //                                     </div>
