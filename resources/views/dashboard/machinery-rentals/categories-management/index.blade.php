...
<div class="container mx-auto">
          ...
          <div class="relative bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
            <div class="flex flex-col items-center justify-between px-4 pt-4 pb-2 space-y-3 lg:flex-row lg:space-y-0 lg:space-x-4">
                <div class="w-full lg:w-1/2">
                    <form action="{{ route('categories-management.index') }}" method="GET">
                        <div class="flex items-center w-full mx-auto mb-0 sm:max-w-screen-sm space-x-2 overflow-hidden">
                            ...
                            <div class="flex-shrink-0">
                                <a href="{{ route('categories-management.index') }}" class="bg-gray-300 hover:bg-gray-500 text-black px-4 py-2 rounded-lg">Reset</a>
                            </div>
                        </div>
                    </form>
                </div>
              ...
              <div class="flex items-stretch justify-between flex-shrink-0 w-full space-y-0 lg:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                <a href="{{ route('categories-management.create') }}">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <th class="flex items-center pr-6 pl-2 py-4 text-gray-900 dark:text-white">
                        ...
                        <img class="w-14 h-14 object-contain rounded border-gray-500 border md:w-16 md:h-16" src="{{ asset('storage/'.$category->category_image) }}" alt="">
                        @elseif($category->category_image_url)
                        <img class="w-14 h-14 object-contain rounded border-gray-500 border md:w-16 md:h-16" src="{{ $category->category_image_url }}" alt="">
                        @else
                        <img class="w-14 h-14 object-contain rounded border-gray-500 border md:w-16 md:h-16" src="{{ asset('img/NoImg.png') }}" alt="">

                        @endif
                        <div class="ps-3 w-44">
                            <div class="text-md font- font-semibold">{{ $category->category_name }}</div>
                            {{-- <div class="font-semibold text-md text-gray-500">a</div> --}}
                        </div>
                    </th>
                    ...
                    <td class="border-x px-2 py-4">
                        <div class="flex items-center justify-center gap-2">
                            ...
                            <a 
                            href="#" 
                            type="button" data-modal-target="readCategoryModal" 
                            data-id="{{ $category->category_slug }}" 
                            data-modal-show="readCategoryModal" 
                            class="font-medium" 
                            ...
                            <a href="{{ route('categories-management.edit', $category->category_slug) }}" class="text-yellow-500 font-bold">
                            ...
                            <form action="{{ route('categories-management.destroy', $category->category_slug) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                    ...
                    <td colspan="9" >
                        <div class="w-full text-center p-6 bg-gray-100 border border-gray-200 rounded-lg shadow-md dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                              We couldn’t find any categories that match your search. Please check back later or explore other keywords.
                            </p>
                          </div>
                    </td>
                </tr>
    ...
    @if(count($categories)>0)
        <div id="readCategoryModal" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 items-center justify-center hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative w-full max-w-2xl max-h-full">
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
                        ...
                        <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="readCategoryModal">
                            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                            </svg>
                    ...
                    <hr class="mb-0">
                    <div class="p-6 space-y-6" id="category-details-content">
                        {{-- Category Details --}}
                    </div>
                </div>
            </div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function() {
        const modal = document.getElementById("readCategoryModal");
        const categoryDetailsContent = document.getElementById("category-details-content");
    
        // Get all buttons with data-modal-target
        const buttons = document.querySelectorAll('[data-modal-target="readCategoryModal"]');
        ...
        if (buttons.length > 0) {
            buttons.forEach(button => {
                button.addEventListener("click", function(event) {
                    ...
                    const editUrl = `/dashboard/services/machinery-rentals/categories-management/${categoryData.category_slug}/edit`;
                    const deleteUrl = `/dashboard/services/machinery-rentals/categories-management/${categoryData.category_slug}`;
                    let imageContent = "";
                    if (categoryData.category_image) {
                        imageContent = `<img class="w-full dark:hidden rounded-md object-cover" src="{{ asset('storage/') }}${categoryData.category_image}" alt="" />`;
                    } else if (categoryData.category_image_url) {
                        imageContent = `<img class="w-full dark:hidden rounded-md object-cover" src="${categoryData.category_image_url}" alt="" />`;
                    } else {
                                    ...
                                    <div class="mb-0 flex items-end  justify-between  gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-start">
                                                <a href="#" class="text-5xl font-extrabold leading-tight text-gray-900 hover:underline dark:text-white justify-start ">${ categoryData.category_name }</a>
                                            </div>
                                        </div>
            ...
            const closeModalButtons = document.querySelectorAll('[data-modal-hide="readCategoryModal"]');
